...
#	app_n= (apprentissage-mean(apprentissage))/sd(apprentissage)
	k= analyse_k_lineaire(apprentissage,horizon)
print(k)	
	matrice= crea_mat(apprentissage,horizon,k)
	modele= lm(y~., data = matrice)
	
	deb= length(apprentissage)-k-horizon+2
	fin= length(serie)
	t=c(deb:fin)

	matpred= crea_mat(serie[t],horizon,k)
	prediction = as.numeric(predict(modele, matpred[,1:k]))
...
pred_reglineaire_difH= function(serie,d){
	...
	for (i in 1:10){
		...
		eqm_t= pred_reglineaire(serie,i,d)
		eqm_v= cbind(eqm_v,eqm_t)
	}

	plot(eqm_v[2:length(eqm_v)], type="l")
	
	return (eqm_v[2:length(eqm_v)])
